...
function generateQRpromptPay($phoneNo = '0988971593', $total = 68) {
    $PromptPayQR = new PromptPayQR();
    $PromptPayQR->size = 8;
    $PromptPayQR->id = $phoneNo;
...
try {
    ...
    // -----------------------
    $payment_type = (string)$dataJson['paymentMethod'];
    switch ($payment_type) {
        case 'promptpay':
            $payment_pic = generateQRpromptPay('0988971539', $total);
            break;
        default:
            $payment_pic = 'bankPay.png';
            break;
    }
    ...
    // -----------------------
    // STEP 15: Insert ecm_orders_bill
    // -----------------------
    $notes = (string)$dataJson['orderNotes'];
    $bill_data = [
        'member_id'    => $userId,
        'order_id'     => $newOrderNo,
        ...
        'address'      => $addresses_detail,
        'province'     => $addresses_provinces,
        'district'     => $addresses_districts,
        'subdistrict'  => $addresses_subdistricts,
        'postcode'     => $addresses_postalCode,
        'notes'        => $notes,
        'timezone'     => $timeZone,
        'created_at'   => $dateNow
    ];
    if (!insertData($conn_cloudpanel, 'ecm_orders_bill', $bill_data)) {
        throw new Exception("Insert ecm_orders_discount failed.");
    }
