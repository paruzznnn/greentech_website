        ...
        <div class="container" style="max-width: 90%;">
            <div class="box-content">
                <div class="social-share">
                    <p><?= ($lang === 'en') ? 'Share this page:' : (($lang === 'cn') ? '分享此页面：' : (($lang === 'jp') ? 'このページを共有する:' : (($lang === 'kr') ? '이 페이지를 공유:' : 'แชร์หน้านี้:'))); ?></p>
                    ...
                    <button class="copy-link-btn" onclick="copyLink()">
                        <?= ($lang === 'en') ? 'Copy Link' : (($lang === 'cn') ? '复制链接' : (($lang === 'jp') ? 'リンクをコピー' : (($lang === 'kr') ? '링크 복사' : 'คัดลอกลิงก์'))); ?>
                    </button>
                </div>
                ...
                <div style="padding-left:50px;">
                    ...
                    <p><?= ($lang === 'en') ? 'Inquire/order Trandar Acoustics products at' : (($lang === 'cn') ? '在以下位置咨询/订购 Trandar Acoustics 产品：' : (($lang === 'jp') ? 'Trandar Acoustics製品のお問い合わせ・ご注文はこちらから' : (($lang === 'kr') ? 'Trandar Acoustics 제품 문의/주문은 다음을 통해 가능합니다' : 'สอบถาม/สั่งซื้อผลิตภัณฑ์ Trandar Acoustics ได้ที่'))); ?></p>
                ...
                <p><?= ($lang === 'en') ? 'Your email will not be displayed to others. Required fields are marked *' : (($lang === 'cn') ? '您的电子邮件不会显示给其他人。必填字段已标记 *' : (($lang === 'jp') ? 'あなたのメールアドレスが他の人に表示されることはありません。必須項目は*でマークされています' : (($lang === 'kr') ? '귀하의 이메일은 다른 사람에게 표시되지 않습니다. 필수 필드는 *로 표시됩니다' : 'อีเมลของคุณจะไม่แสดงให้คนอื่นเห็น ช่องข้อมูลจำเป็นถูกทำเครื่องหมาย *'))); ?></p>
