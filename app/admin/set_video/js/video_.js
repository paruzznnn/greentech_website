$(document).ready(function () {
    ...
    var td_list_videos = new DataTable('#td_list_videos', {
        "autoWidth": false,
        "language": {
            "decimal": "",
        ...
        "serverSide": true,
        ajax: {
            url: "actions/process_video.php",
            method: 'POST',
            dataType: 'json',
            data: function (d) {
                d.action = 'getData_videos';
        ...
        rowCallback: function (row, data, index) {
            ...
            editButton.off('click').on('click', function () {
                ...
                // reDirect('list_videos.php', {
                reDirect('edit_videos.php', {
            ...
            deleteButton.off('click').on('click', function () {
                ...
                Swal.fire({
                    title: "Are you sure?",
                    text: "Do you want to delete the videos?",
                    icon: "warning",
...
$("#submitAddvideos").on("click", function (event) {
    event.preventDefault();

    var formvideos = $("#formvideos")[0];
    var formData = new FormData(formvideos);
    formData.append("action", "addvideos");
    var videosContent = formData.get("videos_content");

    if (videosContent) {
        var tempDiv = document.createElement("div");
        tempDiv.innerHTML = videosContent;
        ...
        formData.set("videos_content", tempDiv.innerHTML);
    }
    ...
    for (var tag of formData.entries()) {
        ...
        if (tag[0] === 'videos_subject' && tag[1].trim() === '') {
            $("#videos_subject").addClass("is-invalid");
            return;
        }
        if (tag[0] === 'videos_description' && tag[1].trim() === '') {
            $("#videos_description").addClass("is-invalid");
            return;
        }
        if (tag[0] === 'videos_content' && tag[1].trim() === '') {
    ...
    if (checkIsUrl) {
        ...
        Swal.fire({
            ...
            text: "Do you want to add videos.!",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#4CAF50",
            cancelButtonColor: "#d33",
...
// $("#submitEditvideos").on("click", function (event) {
//     event.preventDefault();

//     var formvideos = $("#formvideos_edit")[0];
//     var formData = new FormData(formvideos);
//     formData.append("action", "editvideos");
//     var videosContent = formData.get("videos_content");
...
$("#submitEditvideos").on("click", function (event) {
    ...
    var formvideos = $("#formvideos_edit")[0];
    var formData = new FormData(formvideos);
    formData.append("action", "editvideos"); // ใช้ action นี้เพื่อให้ฝั่ง PHP รู้ว่าเป็นแก้ไข
    ...
    let confirmOptions = {
        title: checkIsUrl
            ...
            : "Are you sure?",
        text: "Do you want to edit videos.!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#4CAF50",
        cancelButtonColor: "#d33",
    ...
    Swal.fire(confirmOptions).then((result) => {
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 'success') {
                        window.location.href = "admin_video_list"; // ✅ ไปหน้า list
                    }
                },
                error: function (error) {
                    console.log("error", error);
