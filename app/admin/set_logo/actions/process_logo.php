...
// ** ต้องมีสองบรรทัดนี้ ไม่งั้น $conn และ $base_path จะไม่ถูกกำหนด **
...
// ตรวจสอบสิทธิ์การเข้าถึง (ถ้า check_permission.php มีการ exit() เมื่อไม่ได้รับสิทธิ์)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_logo') {
    ...
    $old_image_path = $_POST['old_image_path'] ?? ''; // Path เก่า เช่น /public/img/old_logo.png
    ...
    $image_path_for_db = $old_image_path; // กำหนดให้ Path เก่าเป็นค่าเริ่มต้นสำหรับเก็บใน DB
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        // $old_image_path คือ Path ที่ดึงมาจาก DB ซึ่งเป็น URL Path เช่น /public/img/old.png
        // ต้องแปลงเป็น Physical Path ก่อนลบ
        if ($old_image_path && strpos($old_image_path, 'LOGOTRAND.png') === false) {
            // สร้าง Physical Path จาก URL Path
            // $base_path_prefix คือส่วนของ URL root (e.g. https://www.trandar.com)
            // $old_image_path_relative คือส่วนที่เหลือ (e.g. /public/img/old.png)
            // เราต้องการแค่ /public/img/old.png มาต่อกับ $_SERVER['DOCUMENT_ROOT']
            
            // ตรวจสอบให้แน่ใจว่า $old_image_path เริ่มต้นด้วย /public/img/ หรือ Path ที่สัมพันธ์กัน
            // ถ้า $old_image_path ที่เก็บใน DB เป็น Absolute URL (ไม่ควรเป็น) ต้องปรับการแปลง
            $old_local_file_path = $_SERVER['DOCUMENT_ROOT'] . $old_image_path; 
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $response['new_image_path'] = $image_path_for_db; // ส่ง Path ใหม่กลับไปให้ frontend
        } else {
