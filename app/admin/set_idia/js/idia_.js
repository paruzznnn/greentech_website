$(document).ready(function () {
    ...
    var td_list_idia = new DataTable('#td_list_idia', {
        "autoWidth": false,
        "language": {
            "decimal": "",
        ...
        rowCallback: function (row, data, index) {
            ...
            editButton.off('click').on('click', function () {
                ...
                // reDirect('list_idia.php', {
                reDirect('edit_idia.php', {
                    idia_id: data.idia_id
                });
            });
            ...
            deleteButton.off('click').on('click', function () {
                ...
                Swal.fire({
                    title: "Are you sure?",
                    text: "Do you want to delete the idia?",
                    icon: "warning",
...
$("#submitAddidia").on("click", function (event) {
    event.preventDefault();

    var formidia = $("#formidia")[0];
    var formData = new FormData(formidia);
    formData.append("action", "addidia");
    var idiaContent = formData.get("idia_content");

    if (idiaContent) {
        var tempDiv = document.createElement("div");
        tempDiv.innerHTML = idiaContent;
    ...
    for (var tag of formData.entries()) {
        ...
        if (tag[0] === 'idia_subject' && tag[1].trim() === '') {
            $("#idia_subject").addClass("is-invalid");
            return;
        }
        if (tag[0] === 'idia_description' && tag[1].trim() === '') {
            $("#idia_description").addClass("is-invalid");
            return;
        }
        if (tag[0] === 'idia_content' && tag[1].trim() === '') {
...
//     var formData = new FormData(formidia);
//     formData.append("action", "editidia");
//     var idiaContent = formData.get("idia_content");
...
$("#submitEditidia").on("click", function (event) {
    ...
    // console.log("👉 Start submitEditidia handler");
    ...
    if (contentFromEditor) {
        ...
        formData.set("idia_content", finalContent);
    ...
    if (!$("#idia_subject").val().trim()) {
        $("#idia_subject").addClass("is-invalid");
        console.error("❌ Validation failed: idia_subject is empty");
        return;
    }
    if (!$("#idia_description").val().trim()) {
        $("#idia_description").addClass("is-invalid");
        console.error("❌ Validation failed: idia_description is empty");
        return;
    }
    if (!finalContent.trim()) {
        alertError("Please fill in content information.");
        console.error("❌ Validation failed: idia_content is empty");
        return;
    }

    formData.set("idia_subject", $("#idia_subject").val());
    formData.set("idia_description", $("#idia_description").val());

    // console.log("📤 Form data prepared:", {
    //     idia_id: $("#idia_id").val(),
    //     idia_subject: $("#idia_subject").val(),
    //     idia_description: $("#idia_description").val(),
    //     idia_content: finalContent,
