...
try {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'addshop') {
        ...
        if (isset($shop_array)) {
            ...
            $stmt = $conn->prepare("INSERT INTO dn_shop 
                (subject_shop, description_shop, content_shop, date_create) 
                VALUES (?, ?, ?, ?)");

            $shop_subject = $shop_array['shop_subject'];
            $shop_description = $shop_array['shop_description'];

            $shop_content = mb_convert_encoding($shop_array['shop_content'], 'UTF-8', 'auto');
            ...
            if (isset($_FILES['fileInput']) && $_FILES['fileInput']['error'][0] != 4) {
                ...
                foreach ($fileInfos as $fileInfo) {
                    if ($fileInfo['success']) {
                        ...
                        insertIntoDatabase($conn, 'dn_shop_doc', $fileColumns, $fileValues);
                    } else {
            ...
            if (isset($_FILES['image_files']) && $_FILES['image_files']['error'] != 4) {
                ...
                foreach ($fileInfos as $fileInfo) {
                    if ($fileInfo['success']) {
                        ...
                        $fileColumns = ['shop_id', 'file_name', 'file_size', 'file_type', 'file_path', 'api_path'];
    ...
    } elseif (isset($_POST['action']) && $_POST['action'] == 'editshop') {
        ...
        if (!empty($shop_array['shop_id'])) {
            ...
            //             $fileWhereClause = 'shop_id = ?';
            //             $fileWhereValues = [$shop_id];

            //             updateInDatabase($conn, 'dn_shop_doc', $fileColumns, $fileValues, $fileWhereClause, $fileWhereValues);
            //         } else {
            //             throw new Exception('Error uploading file: ' . $fileInfo['fileName'] . ' - ' . $fileInfo['error']);
            //         }
            //     }
    ...
    foreach ($fileInfos as $fileInfo) {
        if ($fileInfo['success']) {
            ...
            $fileColumns = ['shop_id', 'file_name', 'file_size', 'file_type', 'file_path', 'api_path'];
            $fileValues = [$shop_id, $fileInfo['fileName'], $fileInfo['fileSize'], $fileInfo['fileType'], $fileInfo['filePath'], $picPath];
    ...
    } elseif (isset($_POST['action']) && $_POST['action'] == 'delshop') {
        ...
        $stmt = $conn->prepare("UPDATE dn_shop 
            SET del = ? 
            WHERE shop_id = ?"); // Removed the extra comma here
    ...
    } elseif (isset($_POST['action']) && $_POST['action'] == 'getData_shop') {
        ...
        if (!empty($searchValue)) {
            $whereClause .= " AND (subject_shop LIKE '%$searchValue%')";
        }

        $orderBy = $columns[$orderIndex] . " " . $orderDir;

        $dataQuery = "SELECT shop_id, subject_shop, date_create FROM dn_shop 
                    WHERE $whereClause
                    ORDER BY $orderBy
                    LIMIT $start, $length";
        ...
        $Index = 'shop_id';
        $totalRecords = getTotalRecords($conn, 'dn_shop', $Index);
        $totalFiltered = getFilteredRecordsCount($conn, 'dn_shop', $whereClause, $Index);
