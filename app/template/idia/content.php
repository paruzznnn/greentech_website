...
// --- ADDED: Check for language preference, including 'kr'. Default to Thai ---
...
if ($searchQuery) {
    $sql .= "
    AND (dn.subject_idia LIKE '%" . $conn->real_escape_string($searchQuery) . "%' OR dn.subject_idia_en LIKE '%" . $conn->real_escape_string($searchQuery) . "%' OR dn.subject_idia_cn LIKE '%" . $conn->real_escape_string($searchQuery) . "%' OR dn.subject_idia_jp LIKE '%" . $conn->real_escape_string($searchQuery) . "%' OR dn.subject_idia_kr LIKE '%" . $conn->real_escape_string($searchQuery) . "%')
    ";
}
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        } elseif ($lang === 'kr' && !empty($row['description_idia_kr'])) {
            $description = $row['description_idia_kr'];
        }
    ...
    <div>
        <form method="GET" action="">
            <div class="input-group">
                <input type="text" name="search" class="form-control" value="<?php echo htmlspecialchars($searchQuery); ?>" placeholder="<?php echo $lang === 'en' ? 'Search idia...' : ($lang === 'cn' ? '搜索新闻...' : ($lang === 'jp' ? 'ニュースを検索...' : ($lang === 'kr' ? '뉴스 검색...' : 'ค้นหาข่าว...'))); ?>">
...
<div class="pagination">
    <?php if ($page > 1): ?>
        <a href="?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($searchQuery); ?>&lang=<?php echo $lang; ?>">
          <?php echo $lang === 'en' ? 'Previous' : ($lang === 'cn' ? '上一页' : ($lang === 'jp' ? '前へ' : ($lang === 'kr' ? '이전' : 'ก่อนหน้า'))); ?>
        </a>
    <?php endif; ?>
    ...
    <?php if ($page < $totalPages): ?>
        <a href="?page=<?php echo $page + 1; ?>&search=<?php echo urlencode($searchQuery); ?>&lang=<?php echo $lang; ?>">
          <?php echo $lang === 'en' ? 'Next' : ($lang === 'cn' ? '下一页' : ($lang === 'jp' ? '次へ' : ($lang === 'kr' ? '다음' : 'ถัดไป'))); ?>
        </a>
    <?php endif; ?>
</div>